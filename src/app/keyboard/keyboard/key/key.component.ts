...
export class KeyComponent implements OnInit, OnDestroy {
  @Input('capsLock') capsLock$?: BehaviorSubject<boolean>;
  ...
  get isCapsKey(): boolean {
    return this.e === ECHR.caps;
  }
  get hasCase(): boolean {
    return this.e === ECHR.chrCase;
  }
  get isChar(): boolean {
    return this.hasCase || this.e === ECHR.chr;
  }
  ...
  _initkey() {
    ...
    this.isHidden =
      this.e == ECHR.errChr || !this.charInit || this.charInit == '?';
    ...
    switch (this.e) {
      ...
      case ECHR.chrCase:
        ...
        this.keySend = this.keyText = this.caps
          ? this.charInit.toUpperCase()
          : this.charInit;
  ...
  keyPress(event: any) {
    if (this.isCapsKey) {
      ...
      Keyboard.sendKeyboadChar(this.keySend);
